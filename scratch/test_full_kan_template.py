...
def create_kan_template_full():
    ...
    # Let's map substitutions for all three parts!
    ...
    substitutions = {
        ...
        241: [
            ("²æÃ.gÀ«±ÀAPÀgï", "{{TENANT_NAME}}")
        ],
        248: [
            ("gÀÆ.28,000/- (E¥ÀàvÉÛAlÄ ¸Á«gÀ)", "gÀ.{{DEPOSIT_AMOUNT}}/- ({{DEPOSIT_AMOUNT_WORDS}})")
        ],
        249: [
            ("01-04-2026", "{{LEASE_START_DATE}}")
        ],
        251: [
            ("1 (MAzÀÄ) wAUÀ¼ÀÄ", "{{NOTICE_PERIOD_NUM}} ({{NOTICE_PERIOD}})")
        ],
        264: [
            ("01-04-2026", "{{LEASE_START_DATE}}")
        ],
        268: [
            ("\xaa\xc0\xc4v\xc0\xc4\xdb \xaa\xc0i\xc1\xb0\xc3P\xc0g\xc0 \xb8\xc0\xbb", "{{OWNER_SIG_NAMES}}\n\n\xaa\xc0\xc4v\xc0\xc4\xdb \xaa\xc0i\xc1\xb0\xc3P\xc0g\xc0 \xb8\xc0\xbb")
        ],
        274: [
            ("\xaa\xc0\xc4v\xc0\xc4\xdb \xa8s\xc9\xc6\xc3U\xc0\xe5z\xc1g\xc0g\xc0 \xb8\xc0\xbb", "{{TENANT_SIG_NAMES}}\n\n\xaa\xc0\xc4v\xc0\xc4\xdb \xa8s\xc9\xc6\xc3U\xc0\xe5z\xc1g\xc0g\xc0 \xb8\xc0\xbb")
        ]
    }
    ...
    for orig_idx in sorted(list(substitutions.keys()) + [28, 43, 45, 135, 150, 152, 240, 241]):
        new_idx = orig_idx - 25
        ...
        if orig_idx == 28:
            ...
            # Owner address split before \xaa\xc0g\xc0 (ರವರು)
            target_token = " \xaa\xc0g\xc0"
            split_idx = text.find(target_token)
            if split_idx != -1:
                owner_addr_str = text[:split_idx]
                text = text.replace(owner_addr_str, "{{OWNER_ADDRESS}}")
        elif orig_idx == 241:
            # Tenant address split before \xaa\xc0g\xc0 (ರವರು)
            target_token = " \xaa\xc0g\xc0"
            split_idx = text.find(target_token)
            if split_idx != -1:
                tenant_addr_str = text[:split_idx]
    ...
    doc.save(out_file)
    print("Full template saved.")

if __name__ == "__main__":
    create_kan_template_full()
